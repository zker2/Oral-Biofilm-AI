/* =============================================
   TOOTH CHECK! / ORALSCAN AI — merged script
   One model, one upload/camera flow, two skins.
   Mode: 'kid' | 'pro'   ·   Lang: 'en' | 'th'
   ============================================= */
...
/* ─── Text dictionaries, split by mode ─────── */
const T = {
  kid: {
    en: {
      heroTitle: 'How clean are your teeth? 🦷', heroSubtitle: "Take a photo after using your special pink dye. Toby will check for you!",
      tabUpload: 'Use a photo', tabCamera: 'Take a photo',
      dzHint: 'Drop your tooth photo here', chooseFile: 'Choose a photo',
      startCamera: 'Start camera', capture: 'Snap!', retake: 'Retake',
      ...
      toastNoCam: 'Camera not available. Try uploading a photo!', modelWaiting: 'Toby is still waking up… please wait! 😴',
      predictError: "Oops! Toby couldn't read that photo. Try again! 😅",
    },
    th: {
      heroTitle: 'ฟันของเราสะอาดแค่ไหน? 🦷', heroSubtitle: 'ถ่ายรูปหลังใช้สารย้อมคราบฟัน แล้วให้ Toby ช่วยตรวจให้!',
      tabUpload: 'ใช้รูปภาพ', tabCamera: 'ถ่ายรูป',
      dzHint: 'วางรูปฟันของเราที่นี่', chooseFile: 'เลือกรูปภาพ',
      startCamera: 'เปิดกล้อง', capture: 'ถ่าย!', retake: 'ถ่ายใหม่',
      ...
      toastNoCam: 'กล้องไม่พร้อมใช้งาน ลองอัปโหลดรูปแทนนะ!', modelWaiting: 'Toby ยังไม่พร้อม รอแป๊บนะ! 😴',
      predictError: 'อุ๊ย! Toby อ่านรูปนี้ไม่ได้ ลองใหม่นะ! 😅',
    }
  },
  pro: {
    en: {
      heroTitle: 'Know your caries risk in seconds.', heroSubtitle: 'Upload a photo taken after applying disclosing gel. The AI reads your biofilm distribution and returns a calibrated risk score with clinical guidance.',
      ...
      startCamera: 'Start Camera', capture: 'Capture', retake: 'Retake',
      ...
      toastNoImage: 'Please upload or capture an image first.', copiedMsg: 'Summary copied to clipboard!',
      toastNoCam: 'Camera not available. Please use Upload instead.', modelWaiting: 'AI model is still loading. Please wait a moment.',
    ...
    th: {
      heroTitle: 'รู้ระดับความเสี่ยงฟันผุ', heroSubtitle: 'อัปโหลดภาพหลังใช้สารย้อมคราบจุลินทรีย์ ระบบ AI จะประเมินความเสี่ยงของฟันผุและคราบชีวภาพโดยอัตโนมัติ',
      ...
      startCamera: 'เปิดกล้อง', capture: 'ถ่ายรูป', retake: 'ถ่ายใหม่',
      ...
      toastNoImage: 'อัปโหลดหรือถ่ายรูปก่อนนะ', copiedMsg: 'คัดลอกสรุปผลแล้ว!',
      toastNoCam: 'กล้องไม่พร้อมใช้งาน ลองอัปโหลดรูปแทน', modelWaiting: 'โมเดลกำลังโหลด กรุณารอสักครู่',
      predictError: 'ไม่สามารถวิเคราะห์รูปภาพได้ ลองใหม่อีกครั้ง',
      trendUp: 'ความเสี่ยงเพิ่มขึ้นจากการวิเคราะห์ครั้งก่อน', trendDown: 'ดีขึ้น — ความเสี่ยงลดลงจากครั้งก่อน',
      trendSame: 'ระดับความเสี่ยงใกล้เคียงกับครั้งก่อน', trendFirst: 'นี่คือการวิเคราะห์ครั้งแรกของคุณ ติดตามต่อเนื่องเพื่อดูแนวโน้ม',
...
function t(key) {
  return T[state.mode]?.[state.lang]?.[key] ?? T[state.mode]?.en?.[key] ?? T.pro.en[key] ?? key;
}
...
/* ─── Risk content (kid + pro fields per label) ─────── */
const RISK = {
  'Low Risk': {
...
const LABEL_MAP = [{ pattern: /low/i, label: 'Low Risk' }, { pattern: /med/i, label: 'Medium Risk' }, { pattern: /high/i, label: 'High Risk' }];
const RISK_RANK = { 'Low Risk': 1, 'Medium Risk': 2, 'High Risk': 3 };
function normaliseLabel(raw) { for (const { pattern, label } of LABEL_MAP) { if (pattern.test(raw)) return label; } return raw; }
...
const dom = {
  ...
  tobyWrap: el('tobyWrap'), tobyNeutral: el('tobyNeutral'), tobyHappy: el('tobyHappy'), tobyMedium: el('tobyMedium'), tobySad: el('tobySad'),
  heroTitle: el('heroTitle'), heroSubtitle: el('heroSubtitle'), heroEyebrow: el('heroEyebrow'),
  dropZone: el('dropZone'), fileInput: el('fileInput'), dropContent: el('dropContent'),
  previewWrap: el('previewWrap'), previewImg: el('previewImg'), removeBtn: el('removeBtn'), previewFilename: el('previewFilename'),
  scanSweep: el('scanSweep'),
  cameraFeed: el('cameraFeed'), captureCanvas: el('captureCanvas'),
  startCameraBtn: el('startCameraBtn'), captureBtn: el('captureBtn'), retakeBtn: el('retakeBtn'),
  cameraCaptured: el('cameraCaptured'), capturedImg: el('capturedImg'), removeCaptureBtn: el('removeCaptureBtn'),
  analyzeBtn: el('analyzeBtn'), btnSpinner: el('btnSpinner'),
  chipDot: el('chipDot'), chipLabel: el('chipLabel'),
  resultsSection: el('resultsSection'),
  resultStars: el('resultStars'), resultHeadline: el('resultHeadline'), resultMessage: el('resultMessage'),
  kidResultCard: el('kidResultCard'), tipsListKid: el('tipsListKid'),
  scorePanel: el('scorePanel'), recPanel: el('recPanel'), resultLevel: el('resultLevel'), riskPill: el('riskPill'),
  confidenceBars: el('confidenceBars'), ringFill: el('ringFill'), ringPct: el('ringPct'), gsNeedle: el('gsNeedle'),
  recText: el('recText'), recTips: el('recTips'), trendPanel: el('trendPanel'), trendChart: el('trendChart'), trendNote: el('trendNote'),
  historyPanel: el('historyPanel'), historyOverlay: el('historyOverlay'), historyList: el('historyList'),
  historyEmpty: el('historyEmpty'), historyCount: el('historyCount'), historyStats: el('historyStats'),
  toast: el('toast'), langBtn: el('langBtn'), modeBtn: el('modeBtn'),
};
...
function showToby(mood) {
  [dom.tobyNeutral, dom.tobyHappy, dom.tobyMedium, dom.tobySad].forEach(n => n.classList.add('hidden'));
  ...
  (map[mood] ?? dom.tobyNeutral).classList.remove('hidden');
  void dom.tobyWrap.offsetWidth;
...
function switchTab(tab) {
  ...
  if (tab === 'camera' && !state.cameraStream && dom.cameraCaptured.classList.contains('hidden')) {
...
dom.captureBtn.addEventListener('click', () => {
  const w = dom.cameraFeed.videoWidth, h = dom.cameraFeed.videoHeight;
  dom.captureCanvas.width = w; dom.captureCanvas.height = h;
  dom.captureCanvas.getContext('2d').drawImage(dom.cameraFeed, 0, 0, w, h);
  dom.captureCanvas.toBlob(blob => {
    state.capturedBlob = blob;
    dom.capturedImg.src = URL.createObjectURL(blob);
...
dom.retakeBtn.addEventListener('click', () => {
  state.capturedBlob = null; dom.capturedImg.src = '';
  dom.cameraCaptured.classList.add('hidden'); dom.retakeBtn.classList.add('hidden');
  dom.analyzeBtn.disabled = true; startCamera();
});
dom.removeCaptureBtn.addEventListener('click', () => {
  state.capturedBlob = null; dom.capturedImg.src = '';
  dom.cameraCaptured.classList.add('hidden'); dom.retakeBtn.classList.add('hidden');
  dom.startCameraBtn.classList.remove('hidden'); dom.captureBtn.classList.add('hidden');
  dom.analyzeBtn.disabled = true;
});
function stopCameraStream() {
  if (state.cameraStream) { state.cameraStream.getTracks().forEach(tr => tr.stop()); state.cameraStream = null; }
}
...
function resetUpload() {
  state.uploadedFile = null; state.capturedBlob = null;
  dom.previewImg.src = ''; dom.fileInput.value = '';
  dom.previewWrap.classList.add('hidden'); dom.dropContent.classList.remove('hidden');
  dom.previewFilename.textContent = ''; dom.analyzeBtn.disabled = true;
  hideResults(); showToby('neutral');
}
...
async function runAnalysis() {
  ...
  setAnalyzeLoading(true); setScanSweep(true);
  try {
    ...
    setAnalyzeLoading(false); setScanSweep(false);
  }
}
...
function setScanSweep(on) { dom.scanSweep.classList.toggle('active', on); }

/* ─── Render results (both skins, always) ─────── */
function renderResults(predictions, imgSrc) {
  const normalised = predictions.map(p => ({ label: normaliseLabel(p.className), probability: p.probability })).sort((a, b) => b.probability - a.probability);
  const top = normalised[0];
  const cfg = RISK[top.label] ?? RISK['Medium Risk'];

  // Save history (shared across modes)
  const entry = { id: Date.now(), label: top.label, probability: top.probability, imgSrc: (imgSrc || '').slice(0, 2000), timestamp: new Date().toISOString() };
...
function renderKidResult(top, cfg) {
  ...
  dom.tipsListKid.innerHTML = cfg.kid.tips[state.lang].map(tip => `<li><span class="tip-icon" aria-hidden="true">${tip.icon}</span><span>${tip.text}</span></li>`).join('');
}
...
function renderProResult(top, cfg, normalised, entry) {
  ...
  normalised.forEach(({ label, probability }) => {
    ...
    row.innerHTML = `<div class="conf-meta"><span class="conf-label">${localLabel(label)}</span><span class="conf-pct-label">${pct}%</span></div><div class="conf-track"><div class="conf-fill c-${c.key}" data-target="${pct}"></div></div>`;
    dom.confidenceBars.appendChild(row);
  });
  requestAnimationFrame(() => requestAnimationFrame(() => {
    dom.confidenceBars.querySelectorAll('.conf-fill[data-target]').forEach(fill => { fill.style.width = fill.dataset.target + '%'; });
  }));
...
function renderTrend(latestEntry) {
  ...
  requestAnimationFrame(() => requestAnimationFrame(() => {
    dom.trendChart.querySelectorAll('.trend-bar[data-h]').forEach(bar => { bar.style.height = bar.dataset.h; });
  }));
  const prev = state.history[1];
  const curr = RISK_RANK[latestEntry.label] ?? 2;
  const p = RISK_RANK[prev.label] ?? 2;
  dom.trendNote.textContent = curr > p ? t('trendUp') : curr < p ? t('trendDown') : t('trendSame');
}
...
function renderHistory() {
  ...
  state.history.forEach(entry => {
    ...
    item.innerHTML = `<img class="hi-thumb" src="${entry.imgSrc || ''}" alt="" onerror="this.style.display='none'" /><div class="hi-info"><div class="hi-level">${localLabel(entry.label)}</div><div class="hi-time">${time} · ${pct}%</div></div><span class="hi-tag ${cfg.key}">${localLabel(entry.label)}</span>`;
    dom.historyList.appendChild(item);
  });
  if (state.history.length) {
    const counts = { low: 0, medium: 0, high: 0 };
    state.history.forEach(e => { const c = RISK[e.label]; if (c) counts[c.key]++; });
    dom.historyStats.innerHTML = `<div class="history-stat"><strong>${counts.low}</strong> ${T.pro[state.lang].low}</div><div class="history-stat"><strong>${counts.medium}</strong> ${T.pro[state.lang].medium}</div><div class="history-stat"><strong>${counts.high}</strong> ${T.pro[state.lang].high}</div>`;
  } else { dom.historyStats.innerHTML = ''; }
}

/* ─── Share / copy summary ─────────────── */
el('shareBtn').addEventListener('click', async () => {
  if (!state.lastPredictions) return;
  const normalised = state.lastPredictions.map(p => ({ label: normaliseLabel(p.className), probability: p.probability })).sort((a, b) => b.probability - a.probability);
  const top = normalised[0];
  const cfg = RISK[top.label] ?? RISK['Medium Risk'];
  const pct = Math.round(top.probability * 100);
  const lines = state.mode === 'kid'
    ? [`Tooth Check! — ${new Date().toLocaleString()}`, `Result: ${top.label} (${pct}% confidence)`, cfg.kid.headline.en, '', cfg.kid.tips.en.map(tip => `${tip.icon} ${tip.text}`).join('\n'), '', '(This is an AI screening tool, not a dental diagnosis. Please consult a dentist.)']
    : [`OralScan AI — ${new Date().toLocaleString()}`, `Risk: ${top.label} (${pct}% confidence)`, '', cfg.pro.recommendation.en, '', cfg.pro.tips.en.map(tip => `• ${tip}`).join('\n'), '', '⚠ For educational screening only. Consult a dentist for diagnosis.'];
  try { await navigator.clipboard.writeText(lines.join('\n')); showToast(t('toastCopied') ?? t('copiedMsg')); }
  catch { showToast('Please show the screen to your dentist! 😊'); }
});

/* ─── Restart ──────────────────────────── */
el('restartBtn').addEventListener('click', () => { resetUpload(); switchTab('upload'); window.scrollTo({ top: 0, behavior: 'smooth' }); });
...
dom.modeBtn.addEventListener('click', () => { state.mode = state.mode === 'kid' ? 'pro' : 'kid'; applyMode(); });
dom.langBtn.addEventListener('click', () => { state.lang = state.lang === 'en' ? 'th' : 'en'; applyAll(); });
...
function applyAll() {
  ...
  if (dom.heroEyebrow) dom.heroEyebrow.textContent = t('eyebrow');
  if (el('wfStep1')) { el('wfStep1').textContent = t('wfStep1'); el('wfStep2').textContent = t('wfStep2'); el('wfStep3').textContent = t('wfStep3'); }

  // Upload / camera
  el('tabUploadText').textContent = t('tabUpload');
  el('tabCameraText').textContent = t('tabCamera');
  el('dzHint').textContent = t('dzHint');
  if (el('dzMeta')) el('dzMeta').textContent = t('dzMeta') || '';
  ...
  // Results headers
  if (el('tipsHeadingKid')) el('tipsHeadingKid').textContent = t('tipsHeading');
  if (el('riskLabel')) el('riskLabel').textContent = t('riskLabel');
  if (el('confidenceTitle')) el('confidenceTitle').textContent = t('confTitle');
  if (el('recTitle')) el('recTitle').textContent = t('recTitle');
  if (el('gsLow')) { el('gsLow').textContent = t('gsLow'); el('gsMed').textContent = t('gsMed'); el('gsHigh').textContent = t('gsHigh'); }
  if (el('trendTitle')) el('trendTitle').textContent = t('trendTitle');
  if (el('historyTitle')) el('historyTitle').textContent = t('historyTitle');
  if (el('clearHistoryBtn')) el('clearHistoryBtn').textContent = t('clearAll');
  if (el('historyEmptyText')) el('historyEmptyText').textContent = t('historyEmpty');
  ...
  el('disclaimerStrong').textContent = strong ? strong : '';
  el('disclaimerStrong').style.display = strong ? '' : 'none';
  el('disclaimerBody').textContent = t('disclaimerBody');

  // Re-render live results in new language/mode
  if (!dom.resultsSection.classList.contains('hidden') && state.lastPredictions) {
    ...
    dom.recText.textContent = cfg.pro.recommendation[state.lang];
    dom.recTips.innerHTML = cfg.pro.tips[state.lang].map(tip => `<li>${tip}</li>`).join('');
    dom.confidenceBars.querySelectorAll('.conf-label').forEach((labelEl, i) => { if (normalised[i]) labelEl.textContent = localLabel(normalised[i].label); });
  }
